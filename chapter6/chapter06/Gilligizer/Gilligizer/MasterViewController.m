...
@property (weak, nonatomic) IBOutlet UISegmentedControl *sortTabs;
@end
...
//    NSSortDescriptor *sortDescriptor = [[NSSortDescriptor alloc] initWithKey:@"showTime" ascending:YES];
    NSString *sortKey = self.sortTabs.selectedSegmentIndex == 0 ? @"showTime": @"title";
...
- (void)configureCell:(UITableViewCell *)cell atIndexPath:(NSIndexPath *)indexPath
{
    Show *object = [self.fetchedResultsController objectAtIndexPath:indexPath];
    cell.textLabel.text = object.title;
}

#pragma mark - dataControl

- (IBAction)tappedSegmentedControl:(id)sender {
    self.fetchedResultsController = nil; //Set to nill so that it can be forced to be recreated
    [self.tableView reloadData];
}
